...
Y.extend(Y.M.atto_image.EditableImg, Y.Base, {
    ...
    recalculateImgWrapper: function() {
         ...
         * top offset.
         *
         * Note: Due to left/right being cleaned up, margin-left/margin-top will be used instead. Kinda of a hack, but I
         *       can't think of problems arising due to using margins instead of the left/right.
         */
        this.nodeWrapper.get('children').each(function(node) {
            node.setStyles({
                marginLeft: (nodeRect.width - this.getImgSize().width) / 2,
                marginTop: (nodeRect.height - this.getImgSize().height) / 2
            });
