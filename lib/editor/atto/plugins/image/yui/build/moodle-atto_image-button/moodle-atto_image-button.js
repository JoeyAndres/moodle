...
Y.extend(Y.M.atto_image.EditableImg, Y.Base, {
    ...
    setSize: function(size) {
        ...
        this.fire(Y.M.atto_image.EditableImg.NAME + ':transform');
    },
    ...
    getRotation: function() {
        return Y.M.atto_image.utility.getRotateNode(this.node);
    },
    ...
    setRotation: function(angle) {
        Y.M.atto_image.utility.rotateNode(this.node, angle);
    ...
    recalculateImgWrapper: function() {
        var nodeRect = this.node.getDOMNode().getBoundingClientRect();
        this._updateOrientation();
        ...
        this.node.setStyles({
            left: (nodeRect.width - this.node.getAttribute('width')) / 2,
            top: (nodeRect.height - this.node.getAttribute('height')) / 2
        });
        ...
        this.fire(Y.M.atto_image.EditableImg.NAME + ':recalculated');
        this.select();
    },
    ...
    _publishEvents: function() {
        /**
         * @event atto_image_EditableImg:click Fired when at least one of the nodes inside image wrapper div is clicked.
        ...
        /**
         * @event atto_image_resizable:dblclick Fired when at least one of the nodes inside image wrapper is double
         *                                      clicked.
         */
        ...
        /**
         * @event atto_image_EditableImg:init Fired once at the beginning.
         */
        ...
        this.publish(Y.M.atto_image.EditableImg.NAME + ':transform', {
            emitFacade: true,
            broadcast: 2,  // Global broadcast, just like button clicks.
            context: this
        }, this);

        this.publish(Y.M.atto_image.EditableImg.NAME + ':recalculated', {
            emitFacade: true,
            broadcast: 2,  // Global broadcast, just like button clicks.
            context: this
        }, this);
...
Y.extend(Y.M.atto_image.resizable, Y.Base, {
    ...
    enable: function() {
        ...
        this._editableImg.on([
            Y.M.atto_image.EditableImg.NAME + ':recalculated',
            Y.M.atto_image.EditableImg.NAME + ':transform'
        ], this._onEditableImgRecalculated, this);
    ...
    _setResizeOverlaySize: function(resizableOverlay) {
         ...
         */
        var newWidth = this.node.getAttribute('width') + 'px';
        var newHeight = this.node.getAttribute('height') + 'px';
        var sizeAttrs = {width: newWidth, height: newHeight};
        var orientation = this._editableImg.getOrientation();
        if (orientation == L || orientation == R) {
            sizeAttrs = {width: newHeight, height: newWidth};
