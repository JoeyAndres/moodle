...
Y.M.atto_image.imgWrapperTemplate =
    '<div class="atto-image-wrapper" contenteditable="false"></div>';
Y.M.atto_image.imgInnerWrapperTemplate =
    '<div class="atto-image-inner-wrapper"></div>';
Y.M.atto_image.imageEditableClass = 'atto-image-helper-editable';  // Note: atto-image-helper-* hence deleted after save.
Y.M.atto_image.resizeOverlayNodeTemplate =
    '<div class="atto-image-resize-overlay atto_control {{classes}}"></div>';
Y.M.atto_image.rotateHandleTemplate =
    '<div class="atto-image-rotate-handle-container atto_control" contenteditable="false">' +
    '    <div class="atto-image-rotate-handle-vertical-line"></div>' +
    '    <div class="atto-image-rotate-handle"></div>' +
    '</div>';// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
...
Y.extend(Y.namespace('M.atto_image').resizable, Y.Base, {
    ...
    _createResizeOverlay: function(resizableOverlayNode, nodeToOverlay) {
        var resizableOverlay = new Y.Overlay({
            ...
            width: this.node.getAttribute('width') + 'px',
            height: this.node.getAttribute('height') + 'px',
    ...
    _onResizeEnd: function(e) {
        ...
        // If there was some sort of "transform: rotate(..." css property applied to
        // this._editableImgGhostContainer.ghostNodeContainer, the this._resizableOverlay will have
        // some offset. Thus reset those offsets.
        this._resizableOverlay.get('boundingBox').setStyles({
            left: '0px',
            top: '0px'
        });
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    initializer: function() {
        ...
        Y.M.atto_image.utility.disableImgWrapperContentEditable(this.editor);
        ...
        // todo: Make a move image story. Right now work around is cut this image, then paste it somewhere else.
        ...
        host.on('atto:htmlcleaned', function(e) {
            e.args.html = this._cleanHelperClasses(e.args.html);
            e.args.html = this._removeAttoControlDuringCopyAndPaste(e.args.html);
            e.args.html = this._removeUnwantedCleanUpElements(e.args.html);
            e.args.html = this._disableAttoImageWrapperContentEditable(e.args.html);
        }, this);

        host.on(
            ['atto:pastehtmlstylescleaned', 'atto:pastehtmlclasscleaned'],
            this._exemptAttoImageWrapperFromClassRemoval,
            this
        );
    },
    ...
    /**
     * Callback passed to 'atto:htmlcleaned' event to remove ".atto-image-wrapper .atto_control" elements.
     *
     * @param {String} content HTML content.
     * @returns {String} Modified html string where .atto_control inside .atto-image-wrapper(s) are removed.
     * @private
     */
    _removeAttoControlDuringCopyAndPaste: function(content) {
        var contentNode = Y.Node.create(content);
        ...
        return contentNode.getHTML();
    },

    /**
     * Callback passed to 'atto:htmlcleaned' event to remove ".atto-image-wrapper > [elem]" elements, where [elem]
     * is any unwanted elements inserted during clean.js's operation.
     *
     * @param {String} content HTML content.
     * @returns {String} Modified html string where .atto_control inside .atto-image-wrapper(s) are removed.
     * @private
     */
    _removeUnwantedCleanUpElements: function(content) {
        var contentNode = Y.Node.create(content);
    ...
    /**
     * Callback passed to 'atto:htmlcleaned' event to set all atto-image-wrapper's contenteditable=false
     *
     * @param {String} content HTML content.
     * @returns {String} Modified html string where atto-image-wrapper have contenteditable=false
     * @private
     */
    _disableAttoImageWrapperContentEditable: function(content) {
        var contentNode = Y.Node.create(content);
        ...
        // wrappers.
        contentNode.all('.atto-image-wrapper[contenteditable=false]').setAttribute('contenteditable', false);

        return contentNode.getHTML();
    },

    /**
     * Callback passed to 'atto:pastehtmlclasscleaned' events. It exempts .atto-image-wrapper from class attribute
     * removal.
     *
     * @param {EventFacade} e EventFacade modified by 'atto:pastehtmlclasscleaned'  event.
     * @private
     */
    _exemptAttoImageWrapperFromClassRemoval: function(e) {
        var nodeList = e.args;
        var exemptionNodeList = nodeList.filter(function(node) {
            return Y.one(node).ancestor('.atto-image-wrapper');
        });
    ...
    _initEditableImg: function(image) {
        ...
        this._editableImg = new Y.M.atto_image.EditableImg({
            ...
            after: {
                ...
                init: function() {
                    this.on('atto_image_EditableImg:init', self._editableImgInitHandler, self);
                    this.fire('atto_image_EditableImg:init');
                }
            }
        });
    },
    ...
    _editableImgInitHandler: function(e) {
        ...
        var selection = this.get('host').getSelectionFromNode(editableImg.nodeWrapper);
    ...
    _resizableInitHandler: function(e) {
        ...
        if (newResizable !== this._resizable) {
            ...
            if (isFirefox) {
                 ...
                 */
                this._resizable.getImgWrapper().insert('<span class="atto_control">', 'before');
                this._resizable.getImgWrapper().insert('<span class="atto_control">', 'after');
                var newSelectionRange = window.rangy.createRange();
                newSelectionRange.setStartBefore(this._resizable.getImgWrapper().getDOMNode());
                newSelectionRange.setEndAfter(this._resizable.getImgWrapper().getDOMNode());
                ...
                resizableSelection = [newSelectionRange];
            } else {
                resizableSelection = this.get('host').getSelectionFromNode(this._resizable.getImgWrapper());
            }
     ...
     * @private
     */
    _resizableResizeStartHandler: function(e) {
        var resizable = e.target;
        resizable.node.removeClass(CSS.RESPONSIVE);
    },
    ...
    _initResizable: function(editableImg) {
        ...
        var resizeCfg = {
            ...
            after: {
                ...
                // won't fire. Issue above suggest this solution.
                init: function() {
                    this.on('atto_image_resizable:init', self._resizableInitHandler, self);
                    this.fire('atto_image_resizable:init');
                }
            }
        };
        this._resizable = new Y.M.atto_image.resizable(resizeCfg);
        this._resizable.on('atto_image_resizable:resize:start', this._resizableResizeStartHandler, this);
        this._resizable.on('atto_image_resizable:dblclick', this._displayDialogueWhileResizing, this);
        this._resizable.on('atto_image_resizable:delete', this._resizableDeleteHandler, this);
    },
    ...
    _initRotatable: function(editableImg) {
        var rotataCfg = {
            ...
            after: {
                ...
                // won't fire. Issue above suggest this solution.
                init: function() {
                    this.fire('atto_image_rotatable:init');
                }
            }
        };
        this._rotatable = new Y.M.atto_image.rotatable(rotataCfg);
    },
