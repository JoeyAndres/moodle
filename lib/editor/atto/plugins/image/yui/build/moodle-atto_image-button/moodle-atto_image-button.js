...
Y.extend(Y.namespace('M.atto_image').resizable, Y.Base, {
    ...
    enable: function() {
        ...
        this._editableImg.on(
            Y.M.atto_image.EditableImg.NAME + ':click',
            this._onClick.bind(this)
        );
        this._editableImg.on(
            Y.M.atto_image.EditableImg.NAME + ':dblclick',
        ...
        //this._editableImg.startDeleteMutationObserver();
        this._editableImg.on(
            Y.M.atto_image.EditableImg.NAME + ':delete',
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _beforeCopyCutHandler: function() {
        if (this._resizable) {
            ...
            var newRangeArr = oldRangyArr.map(function(oldRange) {
                ...
                if (this._resizable &&
                    this.get('host').selectionContainsNode(this._resizable.getImgWrapper())) {

                    // See if resize container is start_node or contains start_node.
                    var start = Y.one(oldRange.startContainer).compareTo(this._resizable.getImgWrapper()) ||
                        this._resizable.getImgWrapper().contains(Y.one(oldRange.startContainer)) ||
                        Y.one(oldRange.startContainer).contains(this._resizable.getImgWrapper());
                    var end = Y.one(oldRange.endContainer).compareTo(this._resizable.getImgWrapper()) ||
                        this._resizable.getImgWrapper().contains(Y.one(oldRange.endContainer)) ||
                        Y.one(oldRange.endContainer).contains(this._resizable.getImgWrapper());

                    var resizableSelection = this.get('host').getSelectionFromNode(this._resizable.getImgWrapper())[0];
    ...
    _resizableInitHandler: function(e) {
        ...
        if (newResizable !== this._resizable) {
            ...
            var resizableSelection = this.get('host').getSelectionFromNode(this._resizable.getImgWrapper());
