...
Y.M.atto_image.resizeNodeContainer = '<div class="atto-image-resize-container atto_control {{classes}}" ></div>';
Y.M.atto_image.resizeOverlayNodeTemplate = '<div class="atto-image-resize-overlay atto_control {{classes}}"></div>';// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
 ...
 *
 * Adds image resizing capability.
 */
...
Y.extend(Y.namespace('M.atto_image').resizable, Y.Base, {
    ...
    disable: function() {
        ...
        this._resizableOverlayNode && this._resizableOverlayNode.remove(true);
        this._resizableOverlayNode = null;
        this._ghostNodeContainer && this._ghostNodeContainer.remove(true);  // Destroy it and its child nodes.
    ...
    _createGhostNodeContainer: function() {
         ...
         */
        var nodeDisplayStyle = this._ghostNode.getComputedStyle("display") || 'inline-block';
        ...
        var nodePositionStyle = this._ghostNode.getComputedStyle("position") || 'relative';
    ...
    _onResize: function(e) {
        ...
        this.node.setStyles({ width: newWidth + 'px', height: newHeight + 'px'});
        this._ghostNodeContainer.setStyles({ width: newWidth + 'px', height: newHeight + 'px'});
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _resizableInitHandler: function(e) {
        ...
        if (e.resizable !== this._resizable) {
            ...
            rangy.getSelection().removeAllRanges();  // Deselect all selection.
            var nodeSelection = this.get('host').getSelectionFromNode(this._resizable.node);
            this.get('host').setSelection(nodeSelection);  // Set selection (rangy).
        }
    },
