...
Y.M.atto_image.utility = {
    ...
    /**
     * Normalized a given coord with respect to origin.
...
Y.extend(Y.M.atto_image.EditableImg, Y.Base, {
    ...
    getOriginalImgOffset: function() {
        return {
            left: parseFloat(this.node.getStyle('margin-left')),
            top: parseFloat(this.node.getStyle('margin-top'))
        };
    },

    /**
     * Sets the left/top style value of the image.
     * @param {{left: {Number}, top: {Number}}} offset left/top style values.
     */
    ...
    setSize: function(size) {
        ...
        var newOriginalOffset = {
            marginLeft: oldSizeRatio.width * oldOriginalOffset.left,
            marginTop: oldSizeRatio.height * oldOriginalOffset.top
        };
    ...
    recalculateImgWrapper: function() {
         ...
         * top offset.
         *
         * Note: use margin-left/margin-top is for non atto_control elements. This is because atto_control will gotten
         *       rid of anyway.
         */
        var leftOffset = (nodeRect.width - this.getImgSize().width) / 2;
        var topOffset = (nodeRect.height - this.getImgSize().height) / 2;
        this.innerNodeWrapper.setStyles({marginLeft: leftOffset, marginTop: topOffset});
        this.innerNodeWrapper.setStyles(this.getImgSize());
        this.nodeWrapper.get('children').filter('.atto_control').each(function(node) {
            node.setStyles({left: leftOffset, top: topOffset});
...
Y.extend(Y.M.atto_image.resizable, Y.Base, {
    ...
    _onResizeEnd: function(e) {
        ...
        // Note: this will in turn call _onEditableImgRecalculated. Calling _onEditableImgRecalculated will swap
        //       width/height (depending on orientation) the resizableOverlay and will mess up the
        //       margin-left/margin-top offsets (it's in a different side now that img is rotated). Recalculate again to
        //       factor in the swapped width/height.
    ...
    _onEditableImgRecalculated: function() {
         ...
         */
        var resizableOverlayBoundingBox = this._resizableOverlay.get('boundingBox');
        if (orientation == Y.M.atto_image.resizeHandles.R) {
            Y.M.atto_image.utility.rotateNode(resizableOverlayBoundingBox, imgAngle + 90);
        } else if (
            ...
            Y.M.atto_image.utility.rotateNode(resizableOverlayBoundingBox, imgAngle + 180);
        } else if (orientation == Y.M.atto_image.resizeHandles.L) {
            Y.M.atto_image.utility.rotateNode(resizableOverlayBoundingBox, imgAngle + 270);
        } else {
            Y.M.atto_image.utility.rotateNode(resizableOverlayBoundingBox, imgAngle);
        }
...
Y.extend(Y.M.atto_image.croppable, Y.Base, {
    ...
    /**
     * Call to build the crop scaffolding.
     */
    enable: function() {
        // If scaffolding is already establish, don't do a thing.
        if (this._enable) {
        ...
        // Align again since rotation might have mis-align this_cropOverlayNode to the image.
        ...
        // Make this a method in EditableImage and give this a more meaningful name.
    ...
    /**
     * Call to take down the disable scaffolding.
     */
    disable: function() {
        // If scaffolding is not a yet establish, don't do a thing.
        if (!this._enable) {
    ...
    /**
     * Publish events for Y.M.atto_image.croppable object.
     *
     * @private
     */
    _publishEvents: function() {
    ...
    _createCropOverlay: function (cropOverlayNode, nodeToOverlay) {
        ...
        cropOverlay.resize.before('drag:end', this._onBeforeCropEnd, this);
    ...
    _onCrop: function(e) {
        ...
        // Google doc like resizing. If tl, tr, bl, br resize handles are drag, preserve aspect ratio.
        switch (this._cropOverlay.resize.handle) {
            case Y.M.atto_image.resizeHandles.TL:
            case Y.M.atto_image.resizeHandles.TR:
            case Y.M.atto_image.resizeHandles.BL:
            case Y.M.atto_image.resizeHandles.BR:
                this._cropOverlay.resize.con.set('preserveRatio', true);
                break;
            default:
                this._cropOverlay.resize.con.set('preserveRatio', false);
        }
    ...
    /**
     * Event handler before crop end. This is because this._getCropOverlayOffset is only available before crop end, not
     * on crop end.
     * @param {Y.EventFacade} e Event facade object.
     * @private
     */
    _onBeforeCropEnd: function() {
    ...
    _onEditableImgRecalculated: function() {
        ...
        if(orientation == Y.M.atto_image.resizeHandles.R) {
            ...
            Y.M.atto_image.utility.rotateNode(this._cropOverlay.get('boundingBox'), imgAngle);
        }
    ...
    /**
     * Gets the (x,y) of overlay in viewport. Note this does not factor in rotation
     * (bounding box left/top stays the same).
     * @returns [{Number}, {Number}] First element is x, second is y.
     * @private
     */
    ...
    /**
     * Gets the (x, y) of overlay in viewport, with rotation factored in.
     * Note: Only available on cropOverlay.resize.before('drag:end', ...) not cropOverlay.resize.on('drag:end', ...).
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    /**
     * Creates this._rotatable object for the a given Y.M.atto_image.EditableImg.
     * @param {Y.M.atto_image.EditableImg} editableImg To be attached to this._rotatable
     * @private
     */
    ...
    /**
     * Destroys the given rotatable (usually this._rotatable).
     * @param {Y.M.atto_image.rotatable} rotatable To be deleted.
     * @private
     */
    ...
    /**
     * Destroy this._croppable
     * @param {Y.M.atto_image.croppable} croppable Resizable instance.
     * @private
     */
    ...
    /**
     * Since this._croppable is enabled/disabled on the fly, this method needs to be called to enabled croppable.
     * @private
     */
    ...
    /**
     * Since this._croppable is enabled/disabled on the fly, this method needs to be called to disabled croppable.
     * @private
     */
