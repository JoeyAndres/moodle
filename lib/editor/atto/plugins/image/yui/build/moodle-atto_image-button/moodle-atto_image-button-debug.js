...
Y.M.atto_image.utility = {
     ...
     * @return True if the selection they range represents are equal.
     */
...
Y.extend(Y.namespace('M.atto_image').resizable, Y.Base, {
    ...
    /**
     * The DeleteMutationObserver for self._ghostNodeContainer.
     *
    ...
    enable: function() {
        ...
        // If this.node, delete this._ghostNodeContainer a swell.
        ...
        // If this._ghostNodeContainer is deleted, delete this.node a swell, as if the original image is deleted itself.
        this._ghostNodeContainerDeleteMutationObserver = new Y.M.atto_image.DeleteMutationObserver({
            node: this._ghostNodeContainer,
            deletionCallback: this.deleteNode.bind(this)
        });
    ...
    deleteNode: function() {
        ...
        Y.log('delete', 'debug', 'atto_image:resizable');
    },
     ...
     */
    getAuxiliaryContainer: function() {
        return this._ghostNodeContainer;
    },
    ...
    _publishEvents: function() {
         ...
         */
        this.publish('atto_image_resizable:delete', {
            emitFacade: true,
            broadcast: 2,  // Global broadcast, just like button clicks.
            context: this
        }, this);
    ...
    _createResizeOverlayNode: function() {
         ...
         */
        resizableOverlayNode.before('dragstart', function(e) {
            e.halt(true);
        });
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _beforeCopyCutHandler: function() {
        if (this._resizable) {
            ...
            var newRangeArr = oldRangyArr.map(function(oldRange) {
                ...
                if (this._resizable &&
                    this.get('host').selectionContainsNode(this._resizable.getAuxiliaryContainer())) {

                    // See if resize container is start_node or contains start_node.
                    var start = Y.one(oldRange.startContainer).compareTo(this._resizable.getAuxiliaryContainer()) ||
                        this._resizable.getAuxiliaryContainer().contains(Y.one(oldRange.startContainer)) ||
                        Y.one(oldRange.startContainer).contains(this._resizable.getAuxiliaryContainer());
                    var end = Y.one(oldRange.endContainer).compareTo(this._resizable.getAuxiliaryContainer()) ||
                        this._resizable.getAuxiliaryContainer().contains(Y.one(oldRange.endContainer)) ||
                        Y.one(oldRange.endContainer).contains(this._resizable.getAuxiliaryContainer());

                    var resizableSelection = this.get('host').getSelectionFromNode(this._resizable.getAuxiliaryContainer())[0];
    ...
    _resizableInitHandler: function(e) {
        ...
        if (e.resizable !== this._resizable) {
            ...
            var resizableSelection = this.get('host').getSelectionFromNode(this._resizable.getAuxiliaryContainer());
            this.get('host').setSelection(resizableSelection);  // Set selection (window.rangy).

            // Disable dragging while resizing to avoid dealing with resize helpers all over the place.
            this._resizable.getAuxiliaryContainer().before('dragstart', function(e) {
                e.halt(true);
            });
        }
    },
