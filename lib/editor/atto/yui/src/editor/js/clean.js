...
EditorClean.prototype = {
    ...
    _cleanHTML: function(content) {
        ...
        var rules = [
            ...
            // Remove all atto-helper class. Note that this differs from atto-control in which, elements that have
            // atto-helper are not removed. Just the atto-helper class is removed. Usage of this is to give styling
            // that is ony available in editor and not when saved.
            {regex: /(<[^>]*?class\s*?=\s*?")([^>"]*)(")/gi, replace: function(match, group1, group2, group3) {
                group2 = group2.replace(/(?:^|[\s])[\s]*atto-helper[_a-zA-Z0-9\-]*/gi,"");
